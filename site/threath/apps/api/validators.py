...
def validate_target(target_type, target_id, type_mappings):
    ...
    :rtype: object
    :raises APIException(ERROR_GENERAL_BAD_TYPE): If target type can not be found
    :raises APIException(ERROR_GENERAL_TARGET_NOT_FOUND): If target object could not
        be found
    """
    try:
        ...
        raise api_errors.APIException(api_errors.ERROR_GENERAL_BAD_TYPE)

    try:
        target = klass.objects.get(pk=target_id)
    except klass.DoesNotExist:
        raise api_errors.APIException(api_errors.ERROR_GENERAL_TARGET_NOT_FOUND)
...
def validate_user(user_id):
    ...
    :rtype: object
    :raises APIException(ERROR_USER_NOT_FOUND): If user could not be found
    """
    try:
